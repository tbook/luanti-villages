...
local function harvest_and_replant(self, crop)
	...
	if replacement and not core.is_protected(crop, "") then
		-- The legacy villager pickup callback treats food as breeding input. The
		-- farmer has an implicit seed supply, so remove the mature crop directly
		-- rather than creating food and seed item entities it could later consume.
		core.remove_node(crop)
